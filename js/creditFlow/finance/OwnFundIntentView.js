...
OwnFundIntentView = Ext.extend(Ext.Panel, {
	...
	constructor : function(_cfg) {
		...
		if(typeof(_cfg.isHiddenName) != 'undefined'){
			this.isHiddenName = _cfg.isHiddenName;
		}
		...
		// 调用父类构造
		OwnFundIntentView.superclass.constructor.call(this, {
			name : "OwnFundIntentViewVM_panel",
	...
	initUIComponents : function() {
		...
		var url = (this.isHaveLending == 'yes' ? __ctxPath
				+ "/creditFlow/finance/listSlFundIntent.do" : __ctxPath
		...
		this.gridPanel = new HT.EditorGridPanel({
			...
			columns : [{
							...
							store : new Ext.data.SimpleStore({
								...
								baseParams : {
									payintentPeriod : typeof(this.object) != 'undefined'
											? (this.isFastOr==true?this.object.getCmpByName('slSmallloanProject.payintentPeriod').getValue():this.object.getCmpByName('ownBpFundProject.payintentPeriod').getValue())
											: 0,
									projectId : this.projectId,
									businessType : this.businessType
								}
						...
						align : 'right',
						summaryType : 'sum',
						hidden : this.isHiddenName ? this.isHiddenName :this.isHiddenMoney,
						renderer : function(value, metaData, record, rowIndex,
								colIndex, store) {
							var flag1 = 0; // incomeMoney
							if (record.data.payMoney != 0) { // payMoney
	...
	createRs : function() {
		...
		var payintentPeriod = (typeof(this.object) != 'undefined'
				? this.object
						.getCmpByName('ownBpFundProject.payintentPeriod')
						.getValue()
	...
	autocreate : function() {
		...
		var investorIds = null;
		var accrual=null;
		var yearManagementConsultingOfRate=null;
		var yearFinanceServiceOfRate=null;
		
		if (this.businessType == "SmallLoan") {
			if(this.isFastOr==true){
				...
				isStartDatePay = this.object.getCmpByName("slSmallloanProject.isStartDatePay") .getValue();
				startDate = this.object .getCmpByName("slSmallloanProject.startDate").getValue();
				payintentPerioDate = this.object .getCmpByName("slSmallloanProject.payintentPerioDate").getValue();
				accrualnew = this.object.getCmpByName("slSmallloanProject.accrualnew").getValue();
				...
				dayOfEveryPeriod = this.object.getCmpByName("slSmallloanProject.dayOfEveryPeriod").getValue();
			    ...
			    balanceRate=this.object.getCmpByName("slSmallloanProject.balanceRate").getValue();
			    accrual=this.object.getCmpByName("slSmallloanProject.accrual").getValue();
			    yearAccrualRate=this.object.getCmpByName("slSmallloanProject.yearAccrualRate").getValue();
			     yearManagementConsultingOfRate=this.object.getCmpByName("slSmallloanProject.yearManagementConsultingOfRate").getValue();
			       yearFinanceServiceOfRate=this.object.getCmpByName("slSmallloanProject.yearFinanceServiceOfRate").getValue();
			...
			accrualtype = this.object
					.getCmpByName("ownBpFundProject.accrualtype").getValue();
			payaccrualType = this.object
					.getCmpByName("ownBpFundProject.payaccrualType")
					.getValue();
			startInterestDate=this.object.getCmpByName("ownBpFundProject.startInterestDate").getValue();	
			projectMoney = this.object
					.getCmpByName("ownBpFundProject.ownJointMoney").getValue();
			isStartDatePay = this.object
					.getCmpByName("ownBpFundProject.isStartDatePay")
					.getValue();
			startDate = this.object
					.getCmpByName("ownBpFundProject.startDate").getValue();
			payintentPerioDate = this.object
					.getCmpByName("ownBpFundProject.payintentPerioDate")
					.getValue();
			accrualnew = this.object
					.getCmpByName("ownBpFundProject.accrualnew").getValue();
			managementConsultingOfRate = this.object
					.getCmpByName("ownBpFundProject.managementConsultingOfRate")
					.getValue();
			financeServiceOfRate = this.object
					.getCmpByName("ownBpFundProject.financeServiceOfRate")
					.getValue();
			payintentPeriod = this.object
					.getCmpByName("ownBpFundProject.payintentPeriod")
					.getValue();
			intentDate = this.object
					.getCmpByName("ownBpFundProject.intentDate").getValue();
			dayOfEveryPeriod = this.object
					.getCmpByName("ownBpFundProject.dayOfEveryPeriod")
					.getValue();
			isPreposePayAccrual = this.object
					.getCmpByName("isPreposePayAccrualCheck").getValue();
			isInterestByOneTime = this.object
					.getCmpByName('ownBpFundProject.isInterestByOneTime')
					.getValue();
			dateMode = this.object.getCmpByName('ownBpFundProject.dateMode')
					.getValue();
			dayAccrualRate = this.object
					.getCmpByName('ownBpFundProject.dayAccrualRate')
					.getValue();
			dayManagementConsultingOfRate = this.object
					.getCmpByName('ownBpFundProject.dayManagementConsultingOfRate')
					.getValue();
			investorIds = 	this.object
					.getCmpByName('ownBpFundProject.investorIds')
					.getValue();	
			dayFinanceServiceOfRate = this.object
					.getCmpByName('ownBpFundProject.dayFinanceServiceOfRate')
					.getValue();
					  balanceRate=this.object.getCmpByName("ownBpFundProject.balanceRate").getValue();
					  
			accrual=this.object.getCmpByName("ownBpFundProject.accrual").getValue();
			yearAccrualRate=this.object.getCmpByName("ownBpFundProject.yearAccrualRate").getValue();
			}
			// isPreposePayConsultingCheck=this.object.getCmpByName("ownBpFundProject.isPreposePayConsultingCheck").getValue();
		}
		...
		var params1 = {
			...
			'ownBpFundProject.investorIds':investorIds,
			'calcutype' : calcutype,
			'flag1' : 0,
			"fundType":"custer",
			'projectId' : projId,
			'businessType' : businessType,
			isHaveLending : "yes",
			'ownBpFundProject.yearAccrualRate': yearAccrualRate ,
			'ownBpFundProject.yearManagementConsultingOfRate':yearManagementConsultingOfRate,
			'ownBpFundProject.yearFinanceServiceOfRate':yearFinanceServiceOfRate
	...
	toExcel : function() {
		...
		var vCount = vRecords.length; // 得到记录长度
		if(vCount!=0){
			if(vRecords[0].data.fundIntentId==null){
				Ext.Msg.alert('提示信息','<font color=red><h3> 请先保存！</h3></font>');
			}else{
				var projectId = this.projectId;
				var businessType = this.businessType;
				window.open(__ctxPath+ "/creditFlow/finance/downloadSlFundIntent.do?projectId="
									+ projectId + "&businessType=" + businessType,'_blank');
			}
		}else{
			Ext.Msg.alert('提示信息','<font color=red> 请先生成放款收息表并保存！</font>');
		}
	}
