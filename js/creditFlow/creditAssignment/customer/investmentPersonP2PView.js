 ...
 * @extends Ext.Panel
 */
investmentPersonP2PView = Ext.extend(Ext.Panel, {
	constructor : function(config) {
		Ext.applyIf(this, config);
		this.initUIComponents();
		investmentPersonP2PView.superclass.constructor.call(this, {
					id : 'investmentPersonP2PView',
					height : 450,
					autoScroll : true,
					layout : 'border',
					title : typeof(this.isHiddenSearchPanel)=="undefined"?'投资客户信息':null,
	...
	initUIComponents : function() {
		var isShow = false;
		var rightValue =  isGranted('_investmentPersonP2PView_see_All');
		var isShop = isGranted('_investmentPersonP2PView_see_shop');
		if (RoleType == "control") {
			isShow = true;
		...
		this.store = new Ext.data.JsonStore({
			url : __ctxPath
					+ '/creditFlow/creditAssignment/customer/listInverstCsInvestmentperson.do?isAll='+rightValue+"&isShop="+isShop,
		...
		var tbar = new Ext.Toolbar({
			items : [/*{
				...
				handler : function() {
										 ...
										 * var selected =
										 * this.centerPanel.getSelectionModel().getSelections();
										 * var thisPanel = this.centerPanel; var
										 * len = selected.length; var list = "";
										 * for (var j = 0; j < len; j++) { if (j ==
										 * (len - 1)) { list += selected[j].id; }
										 * else list += selected[j].id + ","; }
										 * if (0 == len) {
										 * Ext.MessageBox.alert('状态',
										 * '请选择一条记录!'); } else {
										 * Ext.MessageBox.confirm('确认删除',
										 * '是否确认删除 选中的<font color=red>' + len + '</font>条记录',
										 * function(btn) { if (btn == 'yes') {
										 * Ext.Ajax.request({ url :__ctxPath +
										 * '/investment/deletePersonCsInvestmentPerson.do',
										 * //+
										 * '/credit/customer/person/deletePersonMessage.do',
										 * method : 'POST', scope : this,
										 * success : function(response,request) {
										 * var
										 * object=Ext.util.JSON.decode(response.responseText)
										 * if(object.flag=='false'){
										 * Ext.ux.Toast.msg('状态',object.msg)
										 * }else{ Ext.ux.Toast.msg('状态',
										 * '删除成功!'); }
	...
	addThirdPayCount:function(){
		...
		if(cardtypevalue=="身份证"){
			...
			Ext.ux.Toast.msg('状态', '开通存管账户支付账户必须选择证件类型为身份证');
			return;
		}
	}
});