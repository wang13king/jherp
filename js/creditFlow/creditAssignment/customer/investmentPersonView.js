	...
	initUIComponents : function() {
		...
		this.store = new Ext.data.JsonStore({
			url : __ctxPath
					...
					+ rightValue+"&isShop="+isShop+"&see=1",
		...
		var tbar = new Ext.Toolbar({
			items : [{
				...
				handler : function() {
					...
					Ext.Ajax.request({
		                   url:  __ctxPath + '/creditFlow/common/getPersonDepCsBank.do',
									 	 ...
									 	 Ext.MessageBox.alert('状态','请选择一条记录!'); 
									  	 ...
									  	 Ext.MessageBox.confirm('确认删除','是否确认删除 选中的<font color=red>' + len + '</font>条记录',
									  		function(btn) { if (btn == 'yes') {
									  			Ext.Ajax.request({ url :__ctxPath +'/creditFlow/creditAssignment/customer/multiDelCsInvestmentperson.do',
		...
		handler : function() {
			...
			if (s <= 0) {
				...
				handler : function() {
					...
					+'&sex='+sex+'&cardnumber='+cardnumber+'&cardtype='+cardtype+'&isShop='+isShop,'_blank');
				}
			}/*, new Ext.Toolbar.Separator({
				hidden : isGranted('_see_contract') ? false : true
				}),{
