...
plMmOrderView = Ext.extend(Ext.Panel, {
		...
		this.searchPanel = new HT.SearchPanel({
			...
			items : [{
						...
						items : [{
									...
									items : [{
												fieldLabel : this.keystr=='UPlan'? 'U计划名称':(this.keystr=='mmplan'? 'D计划名称' :'产品名称'),
												name : 'mmName',
												xtype : 'textfield'
	...
	//		singleSelect : true,
			...
			columns : [{
						...
						header : this.keystr=='UPlan'? 'U计划名称' : (this.keystr=='mmplan'? 'D计划名称' :'产品名称'),
						dataIndex : 'mmName',
						width : 130
					}, {
						header : '购买金额',
	...
	halfAutomatching:function(){
	    ...
	    if (sum > 0) {
			...
			Ext.Ajax.request({
				...
				Ext.ux.Toast.msg('操作信息', '初始快速匹配完成');
				},
				params : {
					ids : ids,
					seachDate : seachDate,
	...
	autoMatching : function() {
		...
		Ext.Ajax.request({
			...
			success : function(response, request) {
				loadMask1.hide();
				Ext.getCmp("PlMmOrderChildrenOrGrid" + keystr).getStore().reload();
				Ext.ux.Toast.msg('操作信息', '系统自动匹配完成');
			},
			params : {
				seachDate : seachDate,
				keystr : this.keystr
