...
PlManageMoneyProduceView = Ext.extend(Ext.Panel, {
	...
	initUIComponents : function() {
		...
		this.topbar = new Ext.Toolbar({
					items : [{
								...
								xtype : 'button',
								scope : this,
								hidden : (isGranted('_plmm_qx') ? (this.type==0 ? false : true) : true),
								handler : this.editjurisdiction
							},/* {
								iconCls : 'btn-add',
								text : '购买',
								xtype : 'button',
	...
	editjurisdiction:function(){
		...
		if (s <= 0) {
			...
			new RoleGrantRightViewj(mmplanId);
			}
...
//	new RoleGrantRightView(id, roleName);
	...
	editRs : function(record) {
		...
		if (s <= 0) {
			...
			if(parseFloat(s[0].data.investedMoney)!=0){
//				Ext.ux.Toast.msg('操作信息', '已正式投入购买使用的产品，不允许修改派息计算有关的字段！');
				Ext.ux.Toast.msg('操作信息', '已正式投入购买使用的产品不允许编辑！');
			}else{
				new PlManageMoneyProduceForm({
