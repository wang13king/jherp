	...
	initUIComponents : function() {
		...
		this.searchPanel = new HT.SearchPanel({
			...
			items : [{
				...
				items : [{
					...
					items : [{
								fieldLabel : keystr=="mmplan"?'计划名称':'产品名称',
								name : 'Q_mmName_S_LK',
								xtype : 'textfield'
		...
		this.gridPanel = new HT.GridPanel({
			...
			fields : [{
					...
					'intentDate','periods','payMoney','loanerRepayMentStatus','bankNum'],
			columns : [{
						...
						header :  keystr=="mmplan"?'计划名称':'产品名称',
	...
	assignInterestMoneyPruduct : function() {
		...
		if(s.length>1){
			...
			if(eval(s[0].data.afterMoney)>=eval(s[0].data.fundType=="liquidatedDamages"?s[0].data.payMoney:s[0].data.incomeMoney)){
				...
				var window_see = new applyRechargeWindows({
					investPeronId : s[0].data.investPersonId,
...
//					isReadOnly : true
		...
		window_see.show();
//				Ext.Msg.confirm('银行卡号确认','是否派息',function(btn){
//						if(btn=="yes"){
//							Ext.Ajax.request({
	...
	assignInterest : function() {
		...
		if(s.length<=0){
		    ...
		    if(keystr='mmplan'){
		 	...
		 	if(!flag){
		        Ext.Msg.alert('操作信息','选择的台账中，有计划未进行授权平台派息，请先进行平台派息授权！');
			    return;
			} 
			var ids = Array();
			var count=0;
