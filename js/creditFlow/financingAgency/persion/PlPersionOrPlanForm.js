 ...
 * @company 智维软件
 */
PlPersionOrPlanForm = Ext.extend(Ext.Window, {
	// 构造函数
	constructor : function(_cfg) {
		Ext.applyIf(this, _cfg);
		// 必须先初始化组件
		this.initUIComponents();
		PlPersionOrPlanForm.superclass.constructor.call(this, {
					id : 'PlPersionOrPlanFormWin',
					layout : 'fit',
		...
		if (this.porProId != null && this.porProId != 'undefined') {
		    ...
		    Ext.Ajax.request({
				...
				success:function(response,opts){
					...
					Ext.getCmp('payIntersetWay').setValue(data.payIntersetWay);//还款方式
					Ext.getCmp('payMoneyTimeType').setValue(data.payAcctualType);//还款周期
					Ext.getCmp('payAcctualType').setValue(data.payAcctualType);//付息方式
					Ext.getCmp('receiverName').setValue(data.receiverName);//债权持有人名称
					Ext.getCmp('reciverP2PAccount').setValue(data.receiverP2PAccountNumber);//债权持有人P2P账号
					
		            this.fillPayWay(data.payIntersetWay);
		            this.fillPayWay1(data.payAcctualType);
				}
			});
