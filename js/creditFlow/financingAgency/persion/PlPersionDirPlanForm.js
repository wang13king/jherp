 ...
 * @company 智维软件
 */
PlPersionDirPlanForm = Ext.extend(Ext.Window, {
	// 构造函数
	constructor : function(_cfg) {
		Ext.applyIf(this, _cfg);
		// 必须先初始化组件
		this.initUIComponents();
		PlPersionDirPlanForm.superclass.constructor.call(this, {
					id : 'PlPersionDirPlanFormWin',
					layout : 'fit',
		...
		this.formPanel = new Ext.form.FormPanel({
			...
			items : [{
						...
						items : [{
									...
									anchor : '100%',
										listeners : {
											...
											var limitMoney=Ext.getCmp('limitMoney').getValue();
											/*	var startMoney=this.getCmpByName('plManageMoneyPlan.startMoney').getValue();
												var limitMoney=this.getCmpByName('plManageMoneyPlan.limitMoney').getValue();*/
												if(limitMoney<startMoney){
													Ext.getCmp('limitMoney').setValue(null);
													Ext.Msg.alert('操作信息','<font color=red>“单笔投资上限”不能低于“单笔投资金额”！<font>')
													return ;
												}
											}
										}
						...
						items : [{
									...
									xtype : (this.readOnly==true?'datefield':'datetimefield'),
									format : 'Y-m-d H:i:s',
									readOnly : this.readOnly,
...
//									dateRangeText:"预售公告时间不能大于开始投标时间"
						...
						items : [{
									...
									store : new Ext.data.SimpleStore({
										autoLoad : true,
										url : __ctxPath
												+ '/creditFlow/multiLevelDic/listByParentId2AreaDic.do',
							...
							//		value : personData==null?null:personData.parentLiveCity,
									...
									listeners : {
										...
										var opr_obj = this.getCmpByName('proArea')
										opr_obj.clearValue();
						...
						items : [{
										...
										listeners : {
											...
											afterrender : function(opr_obj) {
												...
												console.debug(this.getCmpByName('proArea'));
												var v = document.getElementById("proArea").value;
												console.debug(this.getCmpByName('proArea').getValue());
												//console.debug(result.data.parentLiveCity);
												
												console.debug(v);
													var v = record.data.itemId;
													var st = opr_obj.getStore();
											st.on("load", function() {
														opr_obj.setValue(combox.getValue());
													})
			...
			this.formPanel.loadData({
				...
				/*	proArea*/
					...
					if(result.data.rebateType==1){
					 ...
					 if(result.data.parentLiveCity!=''){debugger
					 //	Ext.getCmp("proArea").setValue(result.data.proAreaName);
					 
					 		var arrStore = new Ext.data.SimpleStore({
											url : __ctxPath
													+ '/creditFlow/multiLevelDic/listByParentId2AreaDic.do',
											fields : ['itemId', 'itemName'],
											baseParams:{parentId:result.data.parentLiveCity}
										})
					 	var opr_obj = this.getCmpByName('proArea');
					 	//
										opr_obj.clearValue();
										...
										function test(r) {
											...
											opr_obj.setValue(result.data.proArea);        
         									opr_obj.setRawValue(result.data.proAreaName);  
									//	opr_obj.select(result.data.proArea);
										///	document.getElementById("proArea").value = result.data.parentLiveCity;
			/*	Ext.Ajax.request({
				url : __ctxPath
							+ '/creditFlow/multiLevelDic/listByParentId2AreaDic.do',
				method:"post",
				scope : this,
				params:{parentId:result.data.parentLiveCity},   
		...
		if (this.pdirProId != null && this.pdirProId != 'undefined') {
		    ...
		    Ext.Ajax.request({
				url : __ctxPath
							+ '/creditFlow/financingAgency/persion/getBidInfoBpPersionDirPro.do?pdirProId='
							+ this.pdirProId,
				method:"post",
				scope : this,
				success:function(response,opts){
					...
					Ext.getCmp('pdirProId').setValue(this.pdirProId);
					...
					Ext.getCmp('payMoneyTime').setValue(data.loanLife);
					...
					Ext.getCmp('payAcctualType').setValue(data.payAcctualType);//付息方式
					
		            this.fillPayWay(data.payIntersetWay);
		            this.fillPayWay1(data.payAcctualType,data.custDate);
				}
			});
		}
	...
	save : function() {
		...
		$postForm({
			...
			callback : function(fp, action) {debugger
    ...
    fillPayWay:function(payWay){
   	...
   	fillPayWay1:function(payWay,custDate){
   		if(payWay=='dayPay'){
