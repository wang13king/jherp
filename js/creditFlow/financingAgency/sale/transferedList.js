 ...
 * @company 智维软件
 */
transferedList = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		Ext.applyIf(this, _cfg);
		// 必须先初始化组件
		this.initUIComponents();
		transferedList.superclass.constructor.call(this, {
					id : 'transferedList'+this.mark,
					title:this.mark == 'sale_2' ? '交易成功债权查询' : this.mark == 'sale_5' ? '确认实收手续费' : '交易服务费记录',
					layout : 'border',
	...
	/*    if(this.keystr=="transferFee"){
	    	...
	    	this.topbar = new Ext.Toolbar({
					items : [{
								...
								xtype : 'button',
								scope : this,
								hidden : this.mark == 'sale_5' ? false : true,
								handler : this.confirmtransferFee
							}]
				});
	   /* }*/
		
		if(this.mark == 'sale_2'){//交易成功债权查询（只要是交易成功的都查）
			listtype = 'transferedList';
		}else if(this.mark == 'sale_5'){//确认实收手续费（只查询交易成功但没有确认实收手续费的）
			listtype = 'preTransferFeeStatus1';
		}else if(this.mark == 'sale_7'){//交易费用列表查询
			listtype = 'preTransferFeeStatus2';
		}		
       /*var params1={
      	 listtype:"transferedList"
       };*/
		...
		this.gridPanel = new HT.GridPanel({
			...
			tbar:this.topbar,
			plugins : [summary],
			id : 'transferedListGridPanel'+this.mark,
			//plugins : [this.keystr=="transferFee"?summary:null],
			url : __ctxPath
					+ "/creditFlow/financingAgency/listPlBidSale.do?listtype="+listtype,
			fields : [{
						name : 'id',
						type : 'int'
			...
			columns : [{
						...
						summaryType : 'sum',
							...
							if(null==value){
								...
								if(record.data.changeMoneyType==1){
