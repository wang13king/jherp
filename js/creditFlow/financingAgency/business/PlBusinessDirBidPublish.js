 ...
 * @company 智维软件
 */
PlBusinessDirBidPublish = Ext.extend(Ext.Panel, {
	// 构造函数
	...
	constructor : function(_cfg) {
		...
		PlBusinessDirBidPublish.superclass.constructor.call(this, {
					id : 'PlBusinessDirBidPublish_' + this.Q_proType_S_EQ+ this.Q_state_N_EQ+this.buttonType,
					layout : 'border',
	...
	initUIComponents : function() {
       ...
       var flag=this.Q_state_N_EQ==21 || this.Q_state_N_EQ==22 ||this.Q_state_N_EQ==23;
		...
		this.gridPanel = new HT.GridPanel({
			...
			tbar : this.topbar,
			id : 'PlBusinessDirBidPublishGrid' + this.Q_proType_S_EQ
					+ this.Q_state_N_EQ,
			url : url,
			...
			fields : [{
						...
						mapping : 'bpBusinessDirPro.yearInterestRate'
					}, {
						name : 'proName',
					...
					}, 'proId', 'bidProName', 'bidProNumber', 'bidMoney',
					'bidMoneyScale', 'startMoney', 'riseMoney', 'createtime',
					'updatetime', 'state', 'startInterestType', 'bidStartTime',
					'publishSingeTime','prepareSellTime', 'bidEndTime', 'bidRemark','isStart','bpBusinessDirPro.proId','bpBusinessDirPro.moneyPlanId','proType','bidSchedule'],
			columns : [{
						header : 'bidId',
						align:'center',
						dataIndex : 'bidId',
...
seeRs : function(){
		...
		if (s <= 0) {
			...
			success : function(response, request) {
					...
					var  bidPlanInfoForm=new PlBidPlanInfoForm({
						...
						projectId:record.get('bpBusinessDirPro.proId'),
						fundProjectId:record.get('bpBusinessDirPro.moneyPlanId'),
				...
				params : {
					projectId:record.get('bpBusinessDirPro.moneyPlanId')
				}
			});
	...
	publish : function() {
		...
		if (selectRs.length == 0) {
			...
			var url = __ctxPath+ "/creditFlow/financingAgency/previewPublishPlBidPlan.do"
			var gp = this.gridPanel;
