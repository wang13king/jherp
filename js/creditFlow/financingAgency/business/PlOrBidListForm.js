 ...
 * @company 智维软件
 */
PlOrBidListForm = Ext.extend(Ext.Window, {
	// 构造函数
	constructor : function(_cfg) {
		Ext.applyIf(this, _cfg);
		// 必须先初始化组件
		this.initUIComponents();
		PlOrBidListForm.superclass.constructor.call(this, {
					id : 'PlOrBidListForm',
					layout : 'border',
	...
	initUIComponents : function() {
		...
		this.gridPanel = new HT.GridPanel({
			...
			id : 'PlOrBidListFormGrid',
			url : __ctxPath
					+ "/creditFlow/financingAgency/listPlBidPlan.do?Q_bpBusinessOrPro.borProId_L_EQ=" +this.borProId+"&Q_proType_S_EQ="+this.proType,
			fields : [{
						name : 'bidId',
						type : 'int'
					},{
						name : 'yearInterestRate',
						mapping : 'bpBusinessOrPro.yearInterestRate'
					},{
						name : 'biddingTypeId',
					...
					'publishSingeTime', 'bidEndTime', 'bidRemark','state','prepareSellTime','investMoney'],
			columns : [{
						header : 'bidId',
						dataIndex : 'bidId',
						hidden : true
...
//this.gridPanel.addListener('rowdblclick', this.rowClick);
	...
	createRs : function() {
		var  date=new Date(); 
	var  date2=new Date(this.loanEndTime); 
		var flag=date.between(date,date2);
		if(!flag){
			Ext.ux.Toast.msg('操作信息', '项目已过期 ！');
			return ;
		}
	...
	/*addautomacthRs : function() {
		...
		Ext.MessageBox.confirm('确认发布？', '发布后将不能修改 ', function(btn) {
						if (btn == 'yes') {
		new PlBusinessOrPlanAutomatchForm(
		{ 
			        borProId : datas.borProId,
					proName:datas.proName,
					proNumber:datas.proNumber,
					yearInterestRate:datas.yearInterestRate,
					monthInterestRate:datas.monthInterestRate,
					dayInterestRate:datas.dayInterestRate,
					totalInterestRate:datas.totalInterestRate,
					interestPeriod:datas.interestPeriod,
					payIntersetWay:datas.payIntersetWay,
					bidMoney:datas.bidMoney,
					loanStarTime:datas.loanStarTime,
					loanEndTime:datas.loanEndTime,
					residueMoney:datas.residueMoney,
					obligatoryPersion:datas.obligatoryPersion,
