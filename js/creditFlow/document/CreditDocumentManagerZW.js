Ext.ns('CreditDocumentManagerZW');
/**
 * 分类管理
 * @class CreditDocumentManagerZW
 * @extends Ext.Panel
 */
CreditDocumentManagerZW=Ext.extend(Ext.Panel,{
	constructor:function(config){
		...
		CreditDocumentManagerZW.superclass.constructor.call(this,{
			id : 'CreditDocumentManagerZW'+this.businessType,
			height:450,
			autoScroll:true,
			layout:'border',
			title:'合同模板管理',
	...
	initUIComponents:function(){
		...
		this.topBar =  new Ext.Toolbar({
			items:[/*{
				...
				hidden : this.isGranted('_read_cdm_'+businessType)?false:true,
				scope : this,
				handler : function(){
				...
				iconCls : 'btn-xls',
				hidden : this.isGranted('_output_cdm_'+businessType)?false:true,
				scope : this,
				handler : function(){
					window.open(__ctxPath+'/contract/outputExcelContractHelper.do?businessType='+this.businessType,'_blank');
				}
			}]
		});
			...
			listeners : {
				...
				click : function(node){
					...
					if(grid!=null){
	             		...
	             		grid.getStore().baseParams={parentid: node.id,leaf : node.leaf};
		...
		var tbar=new Ext.Toolbar({
			items:[{
					...
					iconCls:'btn-add',
					scope:this,
					hidden : isGranted('_create_ddm_'+businessType)?false:true,
					handler:function(){
						if(typeof(this.nodeObj) != "undefined"){
							this.addTemplate(this.nodeObj,this.documentTreePanel,this.businessType);
						}else
				...
				},
				{
					text:'修改',
					iconCls:'btn-edit',
					hidden : isGranted('_edit_ddm_'+businessType)?false:true,
					scope:this,
					handler:function(){
				...
				},
				{
					text:'删除',
					iconCls:'btn-del',
					hidden : isGranted('_remove_ddm_'+businessType)?false:true,
					scope:this,
					handler:function(){
						var rows = this.centerPanel.getSelectionModel().getSelections();
						if(rows.length==0){
		...
		this.centerPanel= new HT.GridPanel({
			...
			columns : [{
				...
				renderer : function(value, metaData, record, rowIndex,colIndex, store) {
					if(record.data.fileid==0){
					    return "<img title='上传模板文件' src='"+__ctxPath+"/images/upload-start.png' onclick='uploadTemplate("+value+")'/>";
					}else{
						return "<img title='删除模板文件' src='"+__ctxPath+"/images/reset.png' onclick='deleteTempletFile("+value+")'/>&nbsp;&nbsp;&nbsp;<img title='查看模板需要的要素' src='"+__ctxPath+"/images/watch.png' onclick='seeTempletElement("+value+")'/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img title='查看模板文本内容' src='"+__ctxPath+"/images/upload-start.png' onclick='uploadTemplate("+value+")'/>"
					}
				}
		...
		uploadTemplate = function(id){
			uploadTEMPLATEID = id;
			//@HT_version3.0
			alert("请上传后缀名为docx的word文件!");
			uploadfile('上传合同模板','cs_document_templet.'+id ,1,null ,"*.docx" ,callUploadFun);
		};
		//上传模板会掉修改fileId——
		...
		seeTemplet = function(id){
			...
			Ext.Ajax.request({
				url : __ctxPath+'/contract/validationFileIsExistContractHelper.do',
				method : 'POST',
				success : function(response, request){
			...
			Ext.Ajax.request({
				...
				success : function(response, request){
					var elementCodeStore = new Ext.data.JsonStore( {
						url : __ctxPath+'/contract/findElementTwoContractHelper.do',
						root : 'topics',
	...
	seeTempletSystem : function(value){
		...
		Ext.Ajax.request({
			...
			success : function(response, request){
				var elementCodeStore = new Ext.data.JsonStore( {
					url : __ctxPath + '/contract/findElementListContractHelper.do',
					root : 'topics',
					fields : [ {name : 'code'},{name : 'value'}, {name : 'depict'}],
