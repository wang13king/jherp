			...
			items:[new Ext.form.FormPanel({
				...
				items : [{
					...
					items :[{
						...
						style:'color:red',
						items : [{
									fieldLabel : "上传图片格式需为png、jpg、gif、jpeg并且不能大于5MB",
									labelAlign : 'left',
									anchor : '100%',
								}]
					},{
...
//									if(extendname==".jpg" ||extendname==".JPG"|| extendname==".jpeg"){
								...
								if(extendname.toLowerCase()!=".pdf"){
//								if(extendname.toLowerCase()==".jpg"  ||extendname.toLowerCase()==".jpeg"){
									display.setText('<div style="width:144px; height:84px; margin:10px 0px 0px 10px; padding:1px 1px 1px 1px;"><img src="http://' + host +"/"+webPath+'" ondblclick=showPic("http://'+host+'/'+webPath+'") width =140 height=80  /></div>',false);
								}else if(extendname==".pdf"){
							...
							failure : function(form, action) {
								...
								Ext.ux.Toast.msg('提示',title+'上传失败！'+action.result.msg);	
							}
						});
					}
				}]
...
//下载图片
function getSrc(hiddenId,panelId){
	var  winobj=Ext.getCmp(panelId);
	var panelObj=winobj.get(0);
	var adV=panelObj.getCmpByName(hiddenId).getValue();

	/*    Ext.Ajax.request({
    	url : __ctxPath+'/creditFlow/fileUploads/GetUrlFileForm.do?fileid='+adV,
		method : 'POST',
		scope : this,
		success : function(date) {
			var fileinfo = Ext.util.JSON.decode(date.responseText).fileinfo;
			downloadPic(fileinfo);
		}
});*/
	
	window.open(__ctxPath+'/creditFlow/fileUploads/DownLoadFileForm.do?fileid='+adV,'_blank');
