...
SlContractView = Ext.extend(Ext.Panel, {
	...
	constructor : function(_cfg) {
		...
		if (typeof(_cfg.isHidden) != "undefined") {
			...
			if(typeof(_cfg.slProjcetId)!="undefined") {
		      this.slProjcetId=_cfg.slProjcetId;
		} 
		...
		this.contractcategroy = new Ext.Toolbar({
			items : [{
				...
				handler : function(){
					...
					//loanContract
					if(this.HTLX=='CREDITJKRHT' || this.HTLX=='P2PJKRHT' || this.HTLX=='C2PJKRHT' || this.HTLX=='PA2PJKRZQHT' || this.HTLX=='CA2PJKRZQHT'){debugger
				...
				scope : this,
				handler : function() {
					this.elSign(businessType, piKey)
				}
			}, {
				iconCls : 'selectIcon',
				text : '批量签署',
	...
	elSign : function(businessType, piKey){
		var selected = this.grid_contractPanel.getSelectionModel().getSelected();
		if (selected==undefined) {
			Ext.ux.Toast.msg('状态', '请选择要签章的记录！');
		}else{
			var contractId = selected.get('id');
			var bidPlanId = selected.get('bidPlanId');
			Ext.MessageBox.wait('正在操作','请稍后...');//锁屏
			Ext.Ajax.request({
				url : __ctxPath+'/contract/bidderSignProduceHelper.do',
				method : 'POST',
				success : function(response, request) {
					Ext.ux.Toast.msg('状态', '合同签章成功');
					Ext.MessageBox.hide();//解除锁屏
				},
				failure : function(response) {
					myMask.hide();
					Ext.ux.Toast.msg('状态', '合同签章失败，请重试！');
				},
				params : {
					contractId:contractId,
					bidPlanId : bidPlanId,
					projId : this.projId,
					slProjcetId: this.slProjcetId
				}
			})
		}
	},
