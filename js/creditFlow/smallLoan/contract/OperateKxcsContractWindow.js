...
OperateKxcsContractWindow = Ext.extend(Ext.Window, {
	...
	//生成合同
	createContract:function(){debugger
		...
		if(documenttempletId){
			...
			if(this.isHidden && this.isWDLoan){
				...
				Ext.Ajax.request({
					...
					success : function(response, request) {
						...
						if(obj.success){
							...
							Ext.getCmp('grid_sharteequity').getStore().load();
						}else{
				...
				//2.网贷->合同放款确认->投资人合同生成
				var url=__ctxPath+'/contract/createP2pContractContractHelper.do';
				var args={
		...
		success : function(response, request) {
			...
			if(obj.success == true){
				if(obj.msg){
					...
					Ext.getCmp('grid_sharteequity').getView().refresh();
					Ext.ux.Toast.msg('状态', '合同生成成功！');
