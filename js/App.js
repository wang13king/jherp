...
function commomClick(panel,url,reportKey,reportType){
	...
	var intentDate_GE = parentObj.intentDate_GE;
	parentObj.intentDate_GE=format(parentObj.intentDate_GE,'yyyy-MM-dd');
	var intentDate_LE = parentObj.intentDate_LE;
	parentObj.intentDate_LE=format(parentObj.intentDate_LE,'yyyy-MM-dd');
	var factDate_GE = parentObj.factDate_GE;
	parentObj.factDate_GE=format(parentObj.factDate_GE,'yyyy-MM-dd');
	var factDate_LE = parentObj.factDate_LE;
	parentObj.factDate_LE=format(parentObj.factDate_LE,'yyyy-MM-dd');
	var startinInterestTime_GE = parentObj.startinInterestTime_GE;
	var startinInterestTime_LE = parentObj.startinInterestTime_LE;
	parentObj.startinInterestTime_GE=format(parentObj.startinInterestTime_GE,'yyyy-MM-dd');
	parentObj.startinInterestTime_LE=format(parentObj.startinInterestTime_LE,'yyyy-MM-dd');
	var searchDate_S = parentObj.searchDate_S;
	parentObj.searchDate_S=format(parentObj.searchDate_S,'yyyy-MM-dd');
	var searchDate_E = parentObj.searchDate_E;
	parentObj.searchDate_E=format(parentObj.searchDate_E,'yyyy-MM-dd');
	if(typeof(intentDate_GE) != "undefined" && typeof(intentDate_LE) != "undefined"){
		if(intentDate_GE.getTime()>intentDate_LE.getTime()){//开始时间大于结束时间
		//alert("起始日期不能大于结束日期");
		Ext.ux.Toast.msg('提示信息', '起始日期不能大于结束日期！');
		return;	
	}
	}
	else if(typeof(factDate_GE) != "undefined" && typeof(factDate_LE) != "undefined"){
		if(factDate_GE.getTime()>factDate_LE.getTime()){//开始时间大于结束时间
		//alert("起始日期不能大于结束日期");
		Ext.ux.Toast.msg('提示信息', '起始日期不能大于结束日期！');
		return;	
	}
	}
	else if(typeof(startinInterestTime_GE) != "undefined" && typeof(startinInterestTime_LE) != "undefined"){
		if(startinInterestTime_GE.getTime()>startinInterestTime_LE.getTime()){//开始时间大于结束时间
		//alert("起始日期不能大于结束日期");
		Ext.ux.Toast.msg('提示信息', '起始日期不能大于结束日期！');
		return;	
	}
	...
	else if(typeof(searchDate_S) != "undefined" && typeof(searchDate_E) != "undefined"){
			if(searchDate_S.getTime()>searchDate_E.getTime()){//开始时间大于结束时间
			//alert("起始日期不能大于结束日期");
			Ext.ux.Toast.msg('提示信息', '起始日期不能大于结束日期！');
			return;	
	}
