 ...
 * @company 智维软件
 */
BpActivityManageForm2 = Ext.extend(Ext.Window, {
	// 构造函数
	activityLabel : '',
	label1 : '',
	label2 : '',
	...
	constructor : function(_cfg) {
		...
		BpActivityManageForm2.superclass.constructor.call(this, {
					id : 'BpActivityManageFormWin2_' + this.type,
					layout : 'fit',
					items : this.formPanel,
					modal : true,
					closable:true,
					height :this.type=='integralExchange'?350:450,
					width : 600,
					maximizable : true,
...
//					html:'<div width="80%" style="border-bottom:1 double red;height:20px;"/><font style="color:red">'+this.label1+'</font></div>'
						...
						listeners:{
												 ...
												 select :function(combox){
												  if(combox.getRawValue()=="加息券"){
												  		Ext.getCmp('yuan1').setVisible(false);
												  		Ext.getCmp('yuan2').setVisible(true);
												  }else{
												  		Ext.getCmp('yuan1').setVisible(true);
					...
					columnWidth : this.type == 'coupon' || this.type=='integralExchange'?.44:.5, // 该列在整行中所占的百分比
					layout : "form", // 从上往下的布局
					labelWidth : leftlabel,
					labelAlign :'right',
					style : 'padding-top:10px;',
					items : [{
						...
						allowBlank : false,
						readOnly : this.readOnly,
						name : 'bpActivityManage.startMoney',
						anchor : '100%'
					}]
				},{								
					columnWidth : .03, // 该列在整行中所占的百分比
...
//					html:'<div width="80%" style="border-bottom:1 double red;height:20px;"/><font style="color:red">使用规则</font></div>'
					...
					//	allowBlank : false,
						...
						valueField : 'itemId',
						triggerAction : 'all',
						hidden:this.type == 'coupon'?false:true,
						store : new Ext.data.SimpleStore({
							autoLoad : true,
			...
			this.formPanel.getForm().submit({
				...
				success : function(response, request) {
					...
					if(obj.flag){
						...
						Ext.getCmp('BpActivityManageFormWin2_'+type).close();
					}
