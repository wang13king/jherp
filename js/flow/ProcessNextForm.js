...
ProcessNextForm = Ext.extend(Ext.Panel, {
	...
	constructor : function(config) {
		...
		//是否指定下一节点处理人
		this.reSetNextPanel = new Ext.Panel({
			border : false,
			autoHeight : true,
			layout : 'form',
			labelAlign : "right",
		...
		this.toolbar = new Ext.Toolbar({
						 ...
						 */
			{
					...
					, '-', {
						...
						scope : this,
						hidden:true,//注：因为互联网金融中起息办理流程中项目信息不同的问题导致此功能有问题，需要调整（2016-03-28）
						handler : function() {
						...
						handler : function() {
							...
							for (var i = 0; i < iframes.length; i++) {
								if (iframes[i].id.indexOf("textarea") != -1) {
									textareaIframe = iframes[i];
								}
							}
		...
		ProcessNextForm.superclass.constructor.call(this, {
					...
					items : [ this.bigprogressRunGridPanel,this.formPanel,this.reSetNextPanel, {
								xtype : 'fieldset',
								layout : 'form',
								autoHeight : true,
								collapsed : false,
...
//						});
	...
	nextStep : function() {
		...
		if (this.isFreeJump) {// 若为自由跳转
			...
			if (this.jumpRadioGroup) {// update by lu 2011.12.26
				// 处理任务节点没有下一节点指向的问题。(如企业常规流程：出具担保意向书)
				var selectedItem = this.jumpRadioGroup.getValue();
				if(selectedItem==null){
					Ext.ux.Toast.msg('操作信息', '请选择下一步操作人！');
					return false;
				}
				signalName= selectedItem.getGroupValue();
				nextUserId = selectedItem.ids;
				destName = selectedItem.destName;
			}
		}

		if (this.jumpRadioGroup && destName == '') {// update by lu 2011.12.26
			// 处理任务节点没有下一节点指向的问题。(如企业常规流程：出具担保意向书)
		...
		// 若在流程节点中指定了下一步的执行人员
		if (this.reSetNextPanel != null && this.reSetNextPanel.getCmpByName('flowAssignId')) {
			flowAssignId=this.reSetNextPanel.getCmpByName('flowAssignId').value;
		}
			...
			if (form.isValid()) {// 是合法有效
				...
				if (taskSubmit != null || taskSubmit2 != null) {
					if (this.jumpRadioGroup.items.length > 1) {
						var info = '确定要提交到<span style="color:red;">"'
								+ signalName.replace(/[0-9]/g, '')
								+ '"</span>吗？';
						if (signalName.indexOf("终止") != -1) {
							info = '任务提交到<span style="color:red;">"'
									+ signalName.replace(/[0-9]/g, '')
									+ '"</span>后,项目(任务)将无法恢复,您确认提交吗？';
						}
	...
	getprogressRunGridPanel : function() {
		var level;
		/*
		 * this.expanderFlow = new Ext.ux.grid.RowExpander({ tpl : new
		 * Ext.Template('', '&nbsp;&nbsp;<p><b>意见与说明:</b>{comments}</p>') //
		 * tpl : new Ext.Template( // '<tpl if="{iafeLevel} // >2">','<p><b>意见与说明:</b>','12321','</tpl>' // )
		 * 
		 * });
		 */
	...
	getNoNextTaskRadioCheck : function() {
		...
		/*
		 * this.buttonPanel = new Ext.Panel({ border : false, layout : { type :
		 * 'hbox', pack : 'start', align : 'middle' //
		 * top、middle、bottom：顶对齐、居中、底对齐；stretch：延伸；stretchmax：以最大的元素为标准延伸 },
		 * defaults : { xtype : 'button', width : 120, margins : '5 5 0 0' },
		 * items :[{ //xtype : 'checkbox', //boxLabel : '自由跳转', text : '自由跳转',
		 * scope : this, hidden : false, handler : this.freeJump }, { xtype :
		 * 'button', text : '保存信息', scope : this, iconCls : 'btn-save', handler :
		 * this.saveStep }, { xtype : 'button', text : '提交任务', scope : this,
		 * iconCls : 'btn-transition', handler : this.nextStep }] })
		 */
	...
	getTaskRadioCheck : function(object) {
		...
		for (var i = 0; i < object.data.length; i++) {
			...
			if ('decision' == destType || 'fork' == destType) {
				Ext.Ajax.request({
					...
					success : function(resp, options) {
						...
						if (outers.length > 0) {
							this.userJumpPanel.show();
							for (var i = 0; i < outers.length; i++) {
								nexTask=outers[i][6]?outers[i][1]:null; //@@
								if (!outers[i][6]
										&& !(outers[i][1].indexOf("终止") != -1)
										&& !(outers[i][1].indexOf("结束") != -1)) {
								...
								if (!outers[i][5]) {
									debugger
									if(outers[i][1]=="认购审核"||outers[i][1]=="结束2"){
										var name = outers[i][4].split(',');
									var id = outers[i][3].split(',')
									console.log(name)
									console.log(id)
									for(var m=0;m<name.length;m++ ){
											radioItems.push({
												boxLabel : boxValue+"["+name[m]+"]",// outers[i][1].replace(/[0-9]/g,''),
												ids:id[m],
												style : "margin-left:10px",
												name : 'jumpPath_display_'+ this.taskId,
												inputValue : outers[i][1],//outers[i][1]
												//checked :outers[i][7] ? outers[i][7]==id[m]?true:false:outers[i][6]?true : false,
												checked :outers[i][7] ? outers[i][7]==id[m]?true : false:  false ,
												//checked :outers[i][7] ? outers[i][7]==id[m]?true : false: id[m]==1 ? false : true ,// 缺省第一个选中
												listeners:{
													scope:this,
													check:function(radio,check){
														if(radio.boxLabel.indexOf('信审复审')!=-1){
															if(check){
															}else{
															}
														}
										...
										radioItems.push({
										boxLabel : boxValue+"["+outers[i][4]+"]",// outers[i][1].replace(/[0-9]/g,''),
										style : "margin-left:10px",
										name : 'jumpPath_display_'+ this.taskId,
										inputValue : outers[i][1],//outers[i][1]
										checked :outers[i][6] ? true : false,// 缺省第一个选中
										listeners:{
											scope:this,
											check:function(radio,check){
												if(radio.boxLabel.indexOf('信审复审')!=-1){
							...
							/*this.label=new Ext.form.Label({
								html:'<font style="color:red;padding-left:38px">下一任务处理人：'+outers[0][4]+'</font>'
							});*/
				...
				this.initNextPanel(object.data[0].destination); //@@
				...
				radioItems.push({
					...
					name : 'jumpPath_display_' + this.taskId,
					// inputValue : object.data[i].name,
					inputValue : object.data[i].destination,
					destType : object.data[i].destType,
					destName : object.data[i].destination,
				...
				this.jumpPanel.insert(0, this.jumpRadioGroup); //开了显示决策哦
				this.jumpPanel.doLayout();
				return;
			} else {// 下一节点为普通任务节点
				var outers;
				Ext.Ajax.request({
					...
					scope : this,
					success : function(resp, options) {
						outers = Ext.decode(resp.responseText);
					}
				})
				this.initNextPanel(object.data[0].destination); //@@
				radioItems.push({
					boxLabel : object.data[i].destination+"["+outers[0][4]+"]",
					name : 'jumpPath_display_' + this.taskId,
					// inputValue : object.data[i].name,
					inputValue : object.data[i].destination,
					destType : object.data[i].destType,
	...
	addmsgPanel : function() {
		/*
		 * this.msgPanel = new Ext.Panel({ border : false, labelWidth : 100,
		 * layout : "column", items :[{ layout : 'form', items:[{ xtype :
		 * 'checkbox', boxLabel : '任务提醒', fieldLabel : '任务提醒方式', scope : this,
		 * readOnly : true, checked : true
		 * 
		 * },{ xtype : 'checkbox', boxLabel : '邮件提醒', scope : this, handler :
		 * function(ck, checked) { if (checked) { this.sendMail = true; } else {
		 * this.sendMail = false; } } },{ xtype : 'checkbox', boxLabel : '短信提醒',
		 * scope : this, handler : function(ck, checked) { if (checked) {
		 * this.sendMsg = true; } else { this.sendMsg = false; } } }]}] });
		 */
	...
	initNextPanel:function(nextTaskName){
		Ext.Ajax.request({
			url : __ctxPath+ '/flow/getIsReSetNextProcessActivity.do?taskId='+this.taskId,
			method : 'POST',
			scope :this,
			success : function(response, request) {
				...
				if(obj.flag=='1'){
					...
					this.reSetNextPanel.add(nextPanel);
					this.doLayout();
				}
			}
		})
		...
		// var oppositeType = 0;
		// projectId =
		// this.formPanel.getCmpByName('slSmallloanProject.projectId')
		// .getValue();
		// oppositeType = this.formPanel.get(0).oppositeType;
		// var mainInfo = null;
		// var title = "查看企业信息";
		// if (oppositeType == 306) {
