/**
 * @author
 * @class P2pBannerlinkView
 * @extends Ext.Panel
 * @description [P2pFriendlink]管理
 * @company 智维软件
 * @createtime:
 */
P2pBannerlinkView = Ext.extend(Ext.Panel, {
			// 构造函数
			constructor : function(_cfg) {
				...
				// 调用父类构造
				P2pBannerlinkView.superclass.constructor.call(this, {
							id : 'P2pBannerlinkView',
							title : 'banner管理',
							region : 'center',
			...
			initUIComponents : function() {
				...
				this.gridPanel = new HT.GridPanel({
					...
					// 使用RowActions
					rowActions : true,
					id : 'P2pBannerlinkGrid',
					url : __ctxPath + "/p2p/listP2pBannerlink.do",
					fields : [{
								name : 'id',
								type : 'int'
							}, 'createDate', 'modifyDate', 'banner', 'name','webKey',
							'orderList', 'url','show'],
					columns : [{
								header : 'id',
								dataIndex : 'id',
...
//												}
			...
			cellclick : function(P2pBannerlinkGrid,rowIndex,columnIndex,e) {	      
					var BannerLinkId = P2pBannerlinkGrid.getStore().getAt(rowIndex).get("id");
					var fieldName = P2pBannerlinkGrid.getColumnModel().getDataIndex(columnIndex); //Get field name
					var url = P2pBannerlinkGrid.getSelectionModel().getSelections()[0].data.url;
					var mark1 = url.split("/")[url.split("/").length-2];
					var tablename  =  "p2p_BannerLink";
					var mark = tablename + "." + mark1;
					if("banner"==fieldName){
						P2pBannerLinkShowPic(mark)
					}
...
//				}
			...
			createRs : function() {
				...
				new P2pBannerlinkForm().show();
			},
			// 按ID删除记录
			removeRs : function(id) {
				$postDel({
							url : __ctxPath + '/p2p/multiDelP2pBannerlink.do',
							ids : id,
							grid : this.gridPanel
						});
			},
			...
			editRs1 : function(record) {
			   ...
			   var grid = Ext.getCmp('P2pBannerlinkGrid'); 
