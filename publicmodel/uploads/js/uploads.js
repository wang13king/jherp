 ...
 * 、
 */
uploads = Ext.extend(Ext.Panel, {
	isDisabled : false,
	// 构造函数
	constructor : function(_cfg) {
		debugger
		this.isTrue=_cfg.isTrue
		if(_cfg.state){
			this.state=true;
		}else{
			this.state=false;
		...
		// 调用父类构造
		uploads.superclass.constructor.call(this, {
					region : 'center',
					layout : 'anchor',
	...
	initUIComponents : function() {
		...
		uploadsFileCommon = function(titleName,typeisfile,mark,uploadsSize,projId,businessType,jStore,setname){
			var reloadjStore_Uploads = function(){jStore.reload()};
			uploadReportJS('上传/下载'+titleName+'文件',typeisfile,mark,uploadsSize,'',null,reloadjStore_Uploads,projId,businessType,setname);
		...
		this.uploadsTopBar = new Ext.Toolbar({
				items : [{
						...
						hidden:this.state,
						handler : function() {
							if(Ext.isEmpty(this.projId)){
								Ext.ux.Toast.msg('操作信息', '请先点击保存按钮');
							}else{
			...
			listeners:{
				...
				'cellclick' : function(grid,row,col,e){
					...
					if(fieldName == 'reloadCol'){//重新上传
						this.reUploadForThisPanel(this.titleName,this.typeisfile,this.projId,this.businessType,true,fileid,this.mark,this.templettype,grid,this.setname)
						...
						this.onlineEditFileCommon(this.mark,this.typeisfile,this.projId,this.businessType,this.isDisabledOnlineButton)
					}
				}
			}
		});
...
//			reUploadReportJS('上传/下载"'+titleName+'"',typeisfile,mark,2,null,null,reloadjStore,projId,businessType,fileid,setname);
			reUploadReportJSExt('上传/下载"'+titleName+'"',typeisfile,mark,2,null,null,reloadjStore,projId,businessType,fileid,setname);// update by gao 2013-10-8
		}
